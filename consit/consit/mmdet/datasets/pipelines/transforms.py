...
def get_bbox_fields(results):
    bbox_fields = results.get('bbox_fields', [])
    if 'gt_bboxes' in results and 'gt_bboxes' not in bbox_fields:
...
class LST(object):
    ...
    Args:
        ratio: shuffle ratio, float
        ranges: list; e.g. [0.2,0.8], only split at this interval, random split image and shuffle them
        mode: list; e.g. ['flip','flop']

    Notes:
        If the image is smaller than the crop size, return the original image
    """

    def __init__(self, ratio=0.5, ranges=[0.2,0.8], mode=['flip', 'flop']):
        assert isinstance(ratio, float)
        assert isinstance(ranges, list)
        assert isinstance(mode, list)
        for i in range(len(ranges)):
                assert isinstance(ranges[i], float)
        for i in range(len(mode)):
                assert isinstance(mode[i], str)
        self.ratio = ratio
        self.ranges = ranges
        self.mode = mode
    ...
    def __call__(self, results):
        if np.random.rand(1) > self.ratio:
                results['PS']=False
                results['PS_place']=None
                results['PS_mode']=None
                return results
        img = results['img'].copy()
        results['PS']=True
        h, w, c = img.shape
        seed = np.random.rand(1)[0]
        place = seed *abs(self.ranges[1]-self.ranges[0])+self.ranges[0]
        results['PS_place']=place
        mode = random.choice(self.mode)
        results['PS_mode']=mode
        if mode == 'flip':
                crop_h = h
                ...
                raise NotImplementedError
        results['img'] = img
        for key in get_bbox_fields(results):
            ...
            for i in range(bboxes.shape[0]):
                    ...
                    if (x1 - crop_w + 1 )*(x2 - crop_w + 1)>=0 and (y1 - crop_h + 1)*(y2 - crop_h + 1) >=0:
                            if mode == 'flip':
                                    if x1 - crop_w + 1 < 0:
                                            x1 = x1 + w - crop_w
                                            x2 = x2 + w - crop_w
                                    ...
                                    if y2 - crop_h + 1 > 0:
                                            y1 = y1 - crop_h
                                            y2 = y2 - crop_h
                            tmp_bb.append([x1,y1,x2,y2])
                            if key == 'gt_bboxes':
                                tmp_label.append(results['gt_labels'][i])
                            continue
                    else:
                            if mode == 'flip':
                                    tmp_bb.append([x1 + w - crop_w, y1, w-1, y2])
                                    tmp_bb.append([0, y1, x2-crop_w, y2])
                                    if key == 'gt_bboxes':
                                            tmp_label.append(results['gt_labels'][i])
                                            tmp_label.append(results['gt_labels'][i])
                                    continue
                            elif mode == 'flop':
                                    tmp_bb.append([x1, y1 + h - crop_h, x2, h-1])
                                    tmp_bb.append([x1, 0, x2, y2 - crop_h])
                                    if key == 'gt_bboxes':
                                            tmp_label.append(results['gt_labels'][i])
                                            tmp_label.append(results['gt_labels'][i])
                                    continue
            results[key] = np.array(tmp_bb).astype(np.float32)
            if key == 'gt_bboxes':
                    results['gt_labels'] = np.array(tmp_label).astype(np.int64)

        return results

# bhchen add 07/01/2021
@PIPELINES.register_module()
    ...
    def __call__(self, results):
        ...
        if np.random.rand() < p:
            ...
            if np.random.rand() < 0.5:
                ...
                # 半遮挡（模糊/云层）
                img[y:y + patch_size, x:x + patch_size, :] = img[y:y + patch_size, x:x + patch_size, :] * 0.5
        ### ----------- 局部遮挡结束 --------------------
        if np.random.rand(1) > self.ratio:
                results['PS']=False
                results['PS_place']=None
...
@PIPELINES.register_module()
class ZeroIgnore(object):
    """zero the regions according to the ignore tag

    """

    def __init__(self, ratio=0.5, fill_in=(0,0,0)):
        assert isinstance(ratio, float)
        self.ratio = ratio
        self.fill_in = fill_in
    ...
    def __call__(self, results):
        for key in get_bbox_fields(results):
            if 'gt_bboxes_ignore' == key and len(results[key]) > 0:
                for i in range(len(results[key])):
                    if np.random.rand(1) <= self.ratio:
                        # handle the overlap region with gt_bboxes
                        bboxes = results['gt_bboxes'].copy()
                        x1,y1,x2,y2 = results[key][i]
                        tmp = []
                        for j in range(len(bboxes)):
                            if iou_([x1,y1,x2,y2],bboxes[j]) > 0.0:
                                tmp.append(j)
                        if len(tmp) == 0:
                            results['img'][y1:y2, x1:x2, :] = self.fill_in
